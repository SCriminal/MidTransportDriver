...
- (NSArray *)arySliderDatas{
    if (!_arySliderDatas) {//1匹配中/2已同意/11已拒绝/21已过期
        _arySliderDatas = @[^(){
    ...
    for (int i = 0; i <self.arySliderDatas.count; i++) {
        ...
        sourceVC.refreshAll = ^{
            for (BaseTableVC * tabVC in weakSelf.childViewControllers) {
                if ([tabVC isKindOfClass:[BaseTableVC class]]) {
                    [tabVC refreshHeaderAll];
                }
            }
        };
...
- (BaseNavView *)nav{
    if (!_nav) {
        BaseNavView * nav = [BaseNavView initNavBackTitle:@"我的报价" rightView:nil];
