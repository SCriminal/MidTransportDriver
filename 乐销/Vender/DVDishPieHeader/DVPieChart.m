...
#define CHART_FONT_UP [UIFont systemFontOfSize:F(12)]
#define CHART_FONT_DOWN [UIFont systemFontOfSize:F(14)]
...
- (void)setDataArray:(NSArray *)dataArray{
    if (dataArray) {
        NSMutableArray * aryTerminal = [NSMutableArray array];
        for (DVFoodPieModel * model in dataArray) {
            for (int i = 0; i<aryTerminal.count; i++) {
                DVFoodPieModel * modelTerminal = aryTerminal[i];
                if (model.rate < modelTerminal.rate) {
                    [aryTerminal insertObject:model atIndex:i];
                    break;
                }
            }
            if ([aryTerminal indexOfObject:model] == NSNotFound) {
                [aryTerminal addObject:model];
            }
        }
        _dataArray = aryTerminal;
    }else{
...
-(void)drawRect:(CGRect)rect{
    ...
    if (self.dataArray.count == 0) {
        ...
        UIColor *color = [UIColor colorWithHexString:@"00D9FF"];
...
- (void)drawLineWithPointArray:(NSArray *)pointArray centerArray:(NSArray *)centerArray {
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        NSString *number = model.subTitle;
        ...
        CGFloat numberWidth = 80.f;
        CGFloat numberHeight = CHART_FONT_UP.lineHeight+2;
        
        CGFloat titleWidth = numberWidth;
        CGFloat titleHeight = CHART_FONT_DOWN.lineHeight+2;
        ...
        //指引线上面的数字
        [name drawInRect:CGRectMake(numberX, numberY, numberWidth, numberHeight) withAttributes:@{NSFontAttributeName:CHART_FONT_UP, NSForegroundColorAttributeName:color,NSParagraphStyleAttributeName:paragraph}];
        
        // 指引线下面的title
        [number drawInRect:CGRectMake(titleX, titleY, titleWidth, titleHeight) withAttributes:@{NSFontAttributeName:CHART_FONT_DOWN,NSForegroundColorAttributeName:color,NSParagraphStyleAttributeName:paragraph}];
